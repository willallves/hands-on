...
MODULE_DESCRIPTION("Driver de acesso ao smartunlock (ESP32 com Chip Serial CP2102");
MODULE_LICENSE("GPL");
...
// Executado quando o arquivo /sys/kernel/smartunlock/{led, ldr, threshold} é lido (e.g., cat /sys/kernel/smartunlock/led)
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff);
// Executado quando o arquivo /sys/kernel/smartunlock/{led, ldr, threshold} é escrito (e.g., echo "100" | sudo tee -a /sys/kernel/smartunlock/led)
...
static struct usb_device *smartunlock_device;        // Referência para o dispositivo USB
static uint usb_in, usb_out;                       // Endereços das portas de entrada e saida da USB
static char *usb_in_buffer, *usb_out_buffer;       // Buffers de entrada e saída da USB
static int usb_max_size;                           // Tamanho máximo de uma mensagem USB

// Variáveis para criar os arquivos no /sys/kernel/smartunlock/{led, ldr, threshold}
...
// Cria e registra o driver do smartunlock no kernel
static struct usb_driver smartunlock_driver = {
    .name        = "smartunlock",     // Nome do driver
    .probe       = usb_probe,       // Executado quando o dispositivo é conectado na USB
    .disconnect  = usb_disconnect,  // Executado quando o dispositivo é desconectado na USB
    .id_table    = id_table,        // Tabela com o VendorID e ProductID do dispositivo
};
module_usb_driver(smartunlock_driver);

// Executado quando o dispositivo é conectado na USB
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
    struct usb_endpoint_descriptor *usb_endpoint_in, *usb_endpoint_out;

    printk(KERN_INFO "smartunlock: Dispositivo conectado ...\n");

    // Cria arquivos do /sys/kernel/smartunlock/*
    sys_obj = kobject_create_and_add("smartunlock", kernel_kobj);
    ignore = sysfs_create_group(sys_obj, &attr_group); // AQUI

    // Detecta portas e aloca buffers de entrada e saída de dados na USB
    smartunlock_device = interface_to_usbdev(interface);
...
static void usb_disconnect(struct usb_interface *interface) {
    printk(KERN_INFO "Yuri: Dispositivo desconectado.\n");
    if (sys_obj) kobject_put(sys_obj);      // Remove os arquivos em /sys/kernel/smartunlock
...
static int usb_send_cmd(char *cmd, int param) {
    ...
    printk(KERN_INFO "SmartUnlock: Enviando comando: %s\n", cmd);
    ...
    ret = usb_bulk_msg(smartunlock_device, usb_sndbulkpipe(smartunlock_device, usb_out), usb_out_buffer, strlen(usb_out_buffer), &actual_size, 1000*HZ);
    if (ret) {
        printk(KERN_ERR "SmartUnlock: Erro de codigo %d ao enviar comando!\n", ret);
        return -1;
    }
    ...
    while (retries > 0) {
        // Lê dados da USB
        ret = usb_bulk_msg(smartunlock_device, usb_rcvbulkpipe(smartunlock_device, usb_in), usb_in_buffer, min(usb_max_size, MAX_RECV_LINE), &actual_size, HZ*1000);
        if (ret) {
            printk(KERN_ERR "SmartUnlock: Erro ao ler dados da USB (tentativa %d). Codigo: %d\n", ret, retries--);
            continue;
        }
        ...
        for (i=0; i<actual_size; i++) {
            ...
            if (usb_in_buffer[i] == '\n') {  // Temos uma linha completa
                ...
                printk(KERN_INFO "SmartUnlock: Recebido uma linha: '%s'\n", recv_line);
                ...
                if (!strncmp(recv_line, resp_expected, strlen(resp_expected))) {
                    printk(KERN_INFO "SmartUnlock: Linha eh resposta para %s! Processando ...\n", cmd);
                    ...
                    printk(KERN_INFO "SmartUnlock: Nao eh resposta para %s! Tentiva %d. Proxima linha...\n", cmd, retries--);
...
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff) {
    ...
    printk(KERN_INFO "SmartUnlock: Lendo %s ...\n", attr_name);
...
static ssize_t attr_store(struct kobject *sys_obj, struct kobj_attribute *attr, const char *buff, size_t count) {
    ...
    if (ret) {
        printk(KERN_ALERT "SmartUnlock: valor de %s invalido.\n", attr_name);
        return -EACCES;
    }

    printk(KERN_INFO "SmartUnlock: Setando %s para %ld ...\n", attr_name, value);
    ...
    if (!strcmp(attr_name, "led"))
        ...
        printk(KERN_ALERT "SmartUnlock: o valor do ldr (sensor de luz) eh apenas para leitura.\n");
        return -EACCES;
    }
    if (ret < 0) {
        printk(KERN_ALERT "SmartUnlock: erro ao setar o valor do %s.\n", attr_name);
        return -EACCES;
    }
